...
def runapadj(mtx,node2id,clcoef,between,ngenera):
    ...
    for node,coef in clcoef.items():
        ...
        preference[nodeid] = coef - 2*between[node]
        pstep[nodeid] = 0.01
...
for cluster in rep2members.keys():
    ...
    for member in rep2members[cluster]:
        #between = 0
    ...
    if len(rep2members[cluster])>=10:
        ...
        #if len(betweenh) == 0:
            #continue
        centers,subclusters = runapadj(mtx,node2id,clustering,bc,len(genera))
        ...
        if len(genera) == 1:
            #pure4viz.append([len(genera),mstat,str(betweens.tolist())])
            pure.append(mstat)
        elif len(genera) >=2 :
            #chimeric4viz.append([len(genera),mstat,str(betweens.tolist())])
            chimeric.append(mstat)
...
'''
print(np.mean(pure),np.mean(chimeric))
print(np.median(pure),np.median(chimeric))
print(stats.ttest_ind(pure,chimeric))
print(stats.kruskal(pure,chimeric))
